...
export default function Modal({
  ...
  // 出店者が切り替わったときにスクロール位置を一番上（写真の先頭）に戻す
  useEffect(() => {
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* モーダル上部ヘッダー（店舗カウント ＆ 閉じるボタンのみ） */}
        <div className="flex items-center justify-between px-3.5 sm:px-5 py-2.5 bg-[#FAF6F0]/90 backdrop-blur-xs border-b border-[#3D322B]/20 select-none z-10 flex-shrink-0">
          <div>
            {currentIndex !== undefined && totalCount !== undefined && (
              <span className="text-xs sm:text-sm font-black text-[#59483E] px-1 font-handwriting">
        ...
        {/* スクロール可能なコンテンツ本体（写真 ＆ 詳細文章が一緒に縦スクロール） */}
        <div ref={scrollContainerRef} className="flex-1 overflow-y-auto overscroll-contain">
          {/* 写真ギャラリー（メモ用紙に貼り付けられたポラロイドスナップ写真風） */}
          <div className="relative w-full bg-[#FAF6F0]/40 p-3 sm:p-5 flex items-center justify-center border-b border-[#3D322B]/15">
            <div className="relative w-full max-w-lg bg-white p-2 sm:p-3 pb-4 sm:pb-6 rounded-md shadow-md border border-stone-300 -rotate-0.5">
              ...
              <div className="relative w-full aspect-[4/3] max-h-[42vh] sm:max-h-[50vh] bg-[#FAF6F0] rounded-xs overflow-hidden border border-stone-200">
                <Image
                  src={currentImage}
          ...
          <div className="p-4 sm:p-6">
            ...
            {/* 掲載内容の変更・お問い合わせ */}
            <div className="mt-5 pt-3 border-t border-stone-300/60 flex justify-start pb-2">
              <button
                type="button"
